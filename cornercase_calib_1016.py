...
class OfflineCalib(object):
    ...
    def all_files_abspath(path):
        ...
        for root, dirs, files in os.walk(path):
            root_file_ls = [os.path.join(root, file) for file in files if '.DS_Store' not in file]  # 拼接 → files_abspath
            if root_file_ls:
    ...
    def need_files(self, root_path, board, case_name):
        """
    ...
    def cp_data(self, case_name):
        ...
        master_files = self.need_files(self.master_data_path, 'master', case_name)
        self.logger.info('slave board use data list')
        slave_files = self.need_files(self.slave_data_path, 'slave', case_name)
    ...
    def main(self, loops, j, case_name, ytype, metric, timeout=120):
        ...
        self.logger.info(
            'type: {}, all: {} , pass: {} , fail: {} '.format(ytype, loop, self.success[ytype], self.fail[ytype]))
        self.check_proc()
        self.kill_mock_proc()
